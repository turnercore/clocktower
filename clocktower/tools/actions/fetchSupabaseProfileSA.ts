...
const fetchSupabaseProfileSA = async (userId: string): Promise<ServerActionReturn<Profile>> => {
  ...
  try {
      ...
      const {data: profileData, error: profileError } = await supabase
        ...
        if (!profileData || profileData.length === 0 || profileData[0] === undefined) {
            createNewProfile(userId)
        }

    return { data: profileData[0] as Profile}
  } catch(error) {
    return { error: extractErrorMessage(error, 'Unknown error from fetchSupabaseProfileSA.') }
}
}
