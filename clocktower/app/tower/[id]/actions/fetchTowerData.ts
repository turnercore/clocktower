...
import { z } from 'zod'
import { UUID, TowerData, TowerRowData, ClockData, TowerInitialData } from '@/types'
...
export const fetchTowerData = async (supabase: SupabaseClient, id: UUID): Promise<TowerData> => {
  const { data, error } = await supabase.from('towers').select('*').eq('id', id).single()
